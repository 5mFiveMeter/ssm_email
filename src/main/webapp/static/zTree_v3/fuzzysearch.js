 ...
 * @returns
 */	
 function fuzzySearch(zTreeId, searchField, searchBtn ,isHighLight, isExpand,config){
	...
	$(searchField).on("focus",function () {
		if(!config.is_load){
			//加载全部节点
			getAllNode(zTreeObj.getNodes())
			function getAllNode(arr){
				$.each(arr,function(k,v){
					if(v.children && v.children.length>0){
						getAllNode(v.children)
					}else{
						zTreeObj.reAsyncChildNodes(v,"refresh",true)
					}
				})
			}
			config.is_load = true
